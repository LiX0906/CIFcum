...
crr_WCE <- function(data, lag, constrained,  id, event , start, stop, adm.cens, expos, covariates = NULL, censor) {
  ...
  temp=seq(1:lag)
  pb=smoothCon(s(temp,bs="ps",k=10),data=data.frame(temp))
  ...
  kal <- data.frame(do.call("rbind", lapply(1:length(id), function(i) Wcecalc(data$stop[data$id==id[i]], data$dose[data$id==id[i]],data$stop[data$id==id[i]],pbasis, lag))))
  ...
  if (censor=="IPCW")  {
    ...
    for (j in 1:length(unique(data_cr$id))){
      if (data_cr$fup[j]< max){
      data_cr_impute$weight[data_cr_impute$id==data_cr$id[j]]=1/wmat[data_cr$fup[j],((data_cr$fup[j]+1):max)]
      } }
    ...
    data_ipcw$event[data_ipcw$event==2]=0
  ...
  # Output and save estimated parameters Phi, gamma, aic, effective degree of freedom  
    ...
    # estimated alpha 
    coef.alpha <- coef.Phi %*% apply(pbasis[,1:length(coef.Phi)],2,sum)
    var.alpha <-t(apply(pbasis[,1:length(coef.Phi)],2,sum)) %*% vcov.Phi %*% apply(pbasis[,1:length(coef.Phi)],2,sum)
    
    # estimated weight  
    weight <- pbasis[,1:length(coef.Phi)] %*% coef.Phi/as.numeric(coef.alpha)
    ...
    if (is.null(covariates)){
      est <- list( WCEmat = WCEmat, var.WCEmat=var.WCEmat, est.Phi = coef.Phi, vcovmat.Phi = vcov.Phi, weight=weight, timehaz=timehaz, surv.base=surv.base, est.alpha=coef.alpha, vcovmat.alpha=var.alpha, constrained = constrained, covariates = NULL, ne = n.events, aic = aic, edf=edf)} else {
      est <- list( WCEmat = WCEmat, var.WCEmat=var.WCEmat, est.Phi = coef.Phi, vcovmat.Phi = vcov.Phi, weight=weight, timehaz=timehaz, surv.base=surv.base, est.alpha=coef.alpha, vcovmat.alpha=var.alpha, beta.covariates=covbeta, se.covariates= covse, covariates = covariates, constrained = constrained, ne = n.events, aic = aic, edf=edf)}
